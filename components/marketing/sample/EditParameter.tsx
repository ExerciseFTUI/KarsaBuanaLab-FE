...
const EditParameter: React.FC<TableParameterProps> = ({ regulation, selectedRegulationName }) => {
  ...
  const [addParam, setAddParam] = useState(0)
  ...
  return (
    <div className="w-fit border-2 border-dark_green rounded-xl p-5 items-center justify-center">
          ...
          {sampleData &&
            sampleData.regulations
              ...
              ?.parameters.map((param) => (
                ...
                {/* Add Param Input */}
                {Array.from({ length: addParam }).map((_, index) => (
                <TableRow key={index}>
                  <TableCell>
      ...
      {
        regulation !== 0 && (
          <div onClick={() => {setAddParam((prev) => prev += 1);
          }} className="hover:bg-dark_green hover:text-white hover:cursor-pointer w-full rounded-lg p-2 mt-1 font-semibold flex justify-center bg-light_green"> + Add Regulation </div>
        )
      }
    </div>
  );

...
const LaporanHasilPemeriksaan: FC<LaporanHasilPemeriksaanProps> = ({ title, color, link }) => {
  return (
    <div className="h-screen px-16 space-y-14">
      <h1 className={`text-center text-2xl font-semibold text-${color}`}>{title}</h1>
      <div className="space-y-3">
        <h2 className={`text-${color} text-xl text-center md:text-left`}>Laporan Hasil Pemeriksaan</h2>
      ...
      <div>
        <button className={`w-full bg-${color} text-lg text-ghost_white p-3 rounded-2xl`}>SUBMIT DRAFT</button>
      </div>
    </div>
  );
}

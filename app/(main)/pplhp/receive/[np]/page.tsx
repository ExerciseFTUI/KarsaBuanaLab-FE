...
function filterRequiredFiles(files:any) {
  return files.filter(
    (file:any) =>
...
export default async function Home({ params }: { params: { np: string } }) {
  ...
  const filteredFiles = filterRequiredFiles(linkData? linkData.file:[]);

  // Map the filtered files to the required format for RangkaianPengamananSampelData
  const RangkaianPengamananSampelData = filteredFiles.map((file:any) => ({
    value: file.url,
    label: file.name,
  }));
